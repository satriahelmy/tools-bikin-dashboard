...
### resources.bikindashboard.com (static HTML + JSON)
```
/
  index.html          ← Halaman utama resource hub
  style.css           ← Semua CSS (dipisah untuk kemudahan maintenance)
  resources.json      ← Data semua resource (dikurasi manual)
```
...
## CSS: Inline vs File Terpisah

### Kapan CSS inline (di dalam `<style>` dalam HTML)
- Tool single page yang berdiri sendiri (satu file `index.html`)
- Tool yang tidak berbagi CSS dengan halaman lain

### Kapan CSS dipisah ke file terpisah (`style.css`)
- Halaman yang punya **lebih dari satu file HTML** — misalnya tool multi-page atau halaman dengan data terpisah (JSON)
- Halaman yang CSS-nya kemungkinan sering diupdate secara independen dari markup-nya

### Aturan file CSS terpisah
- Nama file selalu `style.css`, letakkan di root subdomain
- Load di setiap HTML yang memakainya:
```html
<link rel="stylesheet" href="style.css">
```
- Tetap gunakan CSS variables `--bd-*` yang sama — tidak ada perubahan pada token
- JS tetap inline di dalam `<script>` di masing-masing HTML file, kecuali ada logika yang dipakai bersama (gunakan `script.js`)

Setiap subdomain adalah tool mandiri. CSS boleh inline atau file terpisah sesuai aturan di atas (kecuali Laravel yang punya struktur sendiri).
